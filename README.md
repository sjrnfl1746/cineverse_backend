...
### 영화 목록 랜덤 정렬 시 페이지 간 순서가 변경되는 문제

#### 문제

영화 목록이 데이터베이스에 저장된 순서대로 노출되어 사용자에게 매번 동일한 순서로 표시되었습니다.

#### 원인

영화를 다양하게 노출하기 위해 시드값 없이 `RAND()`를 사용하면 쿼리가 실행될 때마다 새로운 정렬 결과가 생성됩니다.

이로 인해 다음 페이지의 콘텐츠를 불러올 때 이전 페이지에서 조회한 콘텐츠가 중복되거나, 일부 콘텐츠가 누락될 가능성이 있었습니다.

#### 해결

QueryDSL에서 MariaDB의 `RAND(seed)` 함수를 호출하고, 동일한 조회 과정에서는 같은 시드값을 사용하도록 구현했습니다.

#### 결과

- 영화를 데이터베이스 등록 순서가 아닌 무작위 순서로 노출
- 동일한 시드값에서 일관된 정렬 결과 제공
- 페이지 이동 시 콘텐츠가 중복되거나 누락되는 문제 방지
- 시드값을 변경하면 새로운 정렬 순서 제공
